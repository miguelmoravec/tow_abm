...
while (x<=q) {
  ...
  if (x %in% pos == TRUE) {
    ...
    if (x == 1) {
      ...
      #changes tow id if over a week has passed
      #this is broken
      else {
...
data2 = data2 %>% 
  arrange(desc(-tow_id)) %>%
  select("MMSI", "PositionTi", "RiverMile", "diff","diff_mag", "tow_id")
...
#deletes tow_ids of only one data point
...
data_upstream = data3  %>% filter(heading == "Upstream") %>%
  group_by(tow_id) %>% summarize(MMSI = MMSI[1], start_mile = dist_start[1], end_mile = dist_end[1], start_time = max_time_start[1], end_time = max_time_end[1], heading = Heading[1], max_dist_trav = max_dist[1])
...
write.csv(up, paste("FOURupstreaming", name, sep=""), row.names=FALSE)
write.csv(down, paste("FOURdownstreaming", name, sep=""), row.names=FALSE)
```
