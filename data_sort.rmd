...
data_upstream = data3  %>% filter(heading == "Upstream") %>% summarize(MMSI = mean(MMSI), start_mile = dist_start[1], end_mile = dist_end[1], start_time = max_time_start[1], end_time = max_time_end[1], heading = Heading[1], max_dist_trav = max_dist[1])
...
data_upstream$start_time = lubridate::round_date(data_upstream$start_time, "15 minutes")
data_upstream$end_time = lubridate::round_date(data_upstream$end_time, "15 minutes")

data_downstream$start_time = lubridate::round_date(data_downstream$start_time, "15 minutes")
data_downstream$end_time = lubridate::round_date(data_downstream$end_time, "15 minutes")
...
write.csv(up, "upstreaming2.csv", row.names=FALSE)
write.csv(down, "downstreaming2.csv", row.names=FALSE) #IMPORTANT!!! open the csv files and remove the quotes from the column headers or they will not pull up correctly in NetLogo
```
